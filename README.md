...
class ZTypeWorld extends World {
  ...
  public World onTick() {
    ...
    // .addToEnd(new InactiveWord(new Utils().generator(rand), rand.nextInt(580) +
    // 10, 50));
...
class Utils {
  ...
  String generator(Random rand) {
    return this.generatorHelp(rand.nextInt(4) + 2, rand, ""); // later uncomment to
    // get
    // word lengths to vary
    // return this.generatorHelp(6, rand, "");
  }

  String generator(Random rand, int level) {
    return this.generatorHelp(rand.nextInt(level + 2) + 1, rand, ""); // later uncomment to
    // get
    // word lengths to vary
    // return this.generatorHelp(6, rand, "");
  }
...
interface ILoWord {
  ...
  // all
  // the active words in a list whos letters start with the given string
 ...
 * this.first.moveWord(int level) ... -- IWord ... this.first.boolean
 * belowScreen() ... -- boolean
 * 
 * 
 * 
